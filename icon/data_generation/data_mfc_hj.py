...
def test_hj():
    ...
    gs = generate_gaussian_process(next(rng), xs, 1, kernel = rbf_circle_kernel_jax, k_sigma = 1.0, k_l = 1.0)
    gs = gs - (dx * jnp.sum(gs, axis = -1, keepdims = True)) # (1, nx)

    init_rho = generate_gaussian_process(next(rng), xs, 1, kernel = rbf_circle_kernel_jax, k_sigma = 1.0, k_l = 1.0) # (1, nx)
