'This file contains the utility functions for the models'
...
def build_diag_block(cond_len, qoi_kv_len, qoi_k_len, compact):
	'build a diagonal block for a pair of demo and quest'
...
def build_bool_sequence(demo_num, mode, shot_num_min = None):
	'build boolean lists for conditions, qoi_kv and qoi_k'
	if mode == 'full':
...
def build_index_integer(cond_len_list, qoi_kv_len_list, qoi_k_len_list):
	'build the index matrix for the transformer, to index the output of the transformer'
	index = []
	num = len(cond_len_list)
	for i in range(num):
...
def plot_model_consts(basic_mask = None, index_matrix = None, out_mask = None):
	'plot the basic mask, index matrix and out mask'
