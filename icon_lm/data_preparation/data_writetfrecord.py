...
def serialize_element(equation, caption, cond_k, cond_v, qoi_k, qoi_v, count):
	...
	if _print:
		...
		print(f"equation: {equation}", flush=True)
		print(f"cond_k.shape: {cond_k.shape}, cond_v.shape: {cond_v.shape}, qoi_k.shape: {qoi_k.shape}, qoi_v.shape: {qoi_v.shape}", flush=True)
...
def write_ode_tfrecord(name, eqn_type, all_params, all_eqn_captions, all_ts, all_cs, all_us, problem_type):
	'Write ODE data to TFRecord format'
	num = all_us[0].shape[0]
	if problem_type == "forward":
		filename = f"{name}_{eqn_type}_forward.tfrecord"
	...
	if problem_type == "inverse":
		filename = f"{name}_{eqn_type}_inverse.tfrecord"
		print("===========" + filename + "===========", flush=True)
		count = 0
		with tf.io.TFRecordWriter(filename) as writer:
			for params, ts_expand, control, traj in zip(all_params, all_ts, all_cs, all_us):
				equation_name = f"{eqn_type}_inverse_{params}"
				count += 1
				if np.sum(traj) != np.nan:
					s_element= serialize_element(equation = equation_name, caption = None,
...
def write_series_tfrecord(name, eqn_type, all_params, all_eqn_captions, all_ts_first, all_us_first, all_ts_second, all_us_second, problem_type):
	'Write series data to TFRecord format'
	if problem_type == "forward":
		filename = f"{name}_{eqn_type}_forward.tfrecord"
		print("===========" + filename + "===========", flush=True)
		count = 0
...
def write_pde_tfrecord(name, eqn_type, all_params, all_eqn_captions, all_xs, all_ks, all_us, problem_type):
	'Write PDE data to TFRecord format'
	if problem_type == "forward":
		filename = f"{name}_{eqn_type}_forward.tfrecord"
		print(f"==========={filename}===========", flush=True)
		count = 0
...
def write_mfc_gparam_hj_tfrecord(name, eqn_type, all_params, all_eqn_captions, all_txs, all_rhos, problem_type, nt, nx):
	'Write MFC data to TFRecord format'
	'''
	txs: (num, nt * nx, 2)
	rhos: (num, nt * nx, 1)
	'''
	...
	if problem_type == "forward11":
		filename = f"{name}_{eqn_type}_forward11.tfrecord"
		print(f"==========={filename}===========", flush=True)
		count = 0
		with tf.io.TFRecordWriter(filename) as writer:
			for params, txs, rhos in zip(all_params, all_txs, all_rhos):
				equation_name = f"{eqn_type}_forward11_{params}"
	...
	if problem_type == "forward12":
		filename = f"{name}_{eqn_type}_forward12.tfrecord"
		print(f"==========={filename}===========", flush=True)
		count = 0
		with tf.io.TFRecordWriter(filename) as writer:
			for params, txs, rhos in zip(all_params, all_txs, all_rhos):
				equation_name = f"{eqn_type}_forward12_{params}"
				cond_k = txs[:, :nx, :]  # initial rho
				cond_v = rhos[:, :nx, :]  # initial rho
	...
	if problem_type == "forward22":
		filename = f"{name}_{eqn_type}_forward22.tfrecord"
		print(f"==========={filename}===========", flush=True)
		count = 0
		with tf.io.TFRecordWriter(filename) as writer:
			for params, txs, rhos in zip(all_params, all_txs, all_rhos):
				equation_name = f"{eqn_type}_forward22_{params}"
...
def write_evolution_tfrecord(seed, eqn_type, all_params, all_eqn_captions, all_xs, all_us, stride, problem_type, file_name):
	...
	with tf.io.TFRecordWriter(file_name) as writer:
		for params, eqn_captions, xs, us in zip(all_params, all_eqn_captions, all_xs, all_us):
			equation_name = f"{eqn_type}_{problem_type}_{params}_{stride}"
			...
			x2 = einshape("k->jkl", xs, j = u2.shape[1], l = 1) # (truncate or step, N, 1)
			...
			if problem_type == 'forward':
				...
				raise NotImplementedError(f"problem_type = {problem_type} is not implemented")


def write_pde_lin2d(name, eqn_type, all_params, all_eqn_captions, all_xts, all_gs, all_uxts, problem_type):
	'Write PDE 2D linear data to TFRecord format'
	if problem_type == 'forward':
...
def write_pde_heat(name, eqn_type, all_params, all_xs, all_us, problem_type, time_step):
	'''
	Write PDE heat equation data to TFRecord format

	Parameters:
		name: str, base filename
		eqn_type: str, type of equation
		all_params: list of parameters for each equation instance
		all_xs: spatial coordinates
		all_us: solution values at different time steps
		problem_type: str, type of problem ('forward' or 'inverse')
		time_step: int, specific time step to use for QOI
	'''
	if problem_type == "forward":
		filename = f"{name}_{eqn_type}_forward.tfrecord"
		print(f"==========={filename}===========", flush=True)
		...
		with tf.io.TFRecordWriter(filename) as writer:
			for params, xs, us in zip(all_params, all_xs, all_us):
				...
				if np.sum(us) != np.nan:
					s_element = serialize_element(equation=equation_name, caption=None,
												  cond_k=cond_k, cond_v=cond_v,
												  qoi_k=qoi_k, qoi_v=qoi_v,
												  count=count)
					writer.write(s_element)
				else:
					raise Exception("NaN found!")
		...
		with tf.io.TFRecordWriter(filename) as writer:
			for params, xs, us in zip(all_params, all_xs, all_us):
				equation_name = f"{eqn_type}_inverse_{params}"
				# Solution at specified time step as condition
				cond_k = xs
				cond_v = us[time_step, :]
				# Initial condition as QOI
				qoi_k = xs
				qoi_v = us[0, :]
				count += 1
				if np.sum(us) != np.nan:
