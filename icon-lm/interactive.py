...
def parse_equation(equation_dict):
    ...
    if "ode" in equation_type:
        ...
        datawrite.write_ode_tfrecord(name = f"{FLAGS.dir}/{name}", eqn_type = equation_type.split("_", 1)[0], 
                        all_params = all_params, all_eqn_captions = all_eqn_captions,
                        all_ts = all_ts, all_cs = all_cs, all_us = all_us,
                        problem_type = equation_type.split("_")[-1])
    
    if "damped_oscillator" in equation_type:
        ts_first = jnp.repeat(domain, demo_num - 1, axis=0)
        ts_second = jnp.repeat(domain + 0.5, demo_num - 1, axis=0)
        decays = jax.random.uniform(next(rng), (demo_num), minval = 0.0, maxval = 2.0)
    ...
    if "poisson" in equation_type:
        ...
        datawrite.write_pde_tfrecord(name = f"{FLAGS.dir}/{name}", eqn_type = equation_type.split("_", 1)[0], 
                      all_params = all_params, all_eqn_captions = all_eqn_captions,
...
def prepare_model_input(demo_num):
...
def solve_differential_equation(equation_string):
    demo_num = parse_equation(equation_string)
    raw, equation, caption, data, label, model_config, test_config = prepare_model_input(demo_num)
...
# domain: x in [0, 1]
# params in [-1, 1]
domain = jnp.linspace(0, 1, 50, endpoint=False)
ode_auto_const = { # example: u'(t) = -0.5c(t) + 0.2
    'equation_type': 'ode_auto_const_forward',
    'domain': domain,
    'parameters': [1.0, 0.2],
    'conditions': {
        'init': [0.0],
        'control': jnp.sin(domain) 
    },
    'demos': 3
}
...
series_damped_oscillator = { # example: u(t) = Asin((2pi/T)t + eta)*e^(0.75t)
    ...
    'domain': jnp.lispace(0, 0.5, 101), # refers to the first half of the domain on which information is provided
    'parameters': [0.75],
    'conditions': {
        'init': [1.0, 0.15, jnp.pi], #A, T, and eta counted as initial conditions for consistency
        'control': jnp.sin(jnp.lispace(0, 0.5, 101)) # counting control as the first half of u(t)
    },
    'demos': 3
}
pde_poisson_spatial = { # example: u''(x) = c(x)
    ...
    'parameters': [],
    'conditions': {
        'init': [0.0, 1.0], # [u_l, u_r]
        'control': 1/2 * jnp.linspace(0.0, 1.0, 101) 
    },
    'demos': 3
}
...
#   for key, value in FLAGS.__flags.items():
  ...
  prediction = solve_differential_equation(pde_poisson_spatial)
  print(prediction)
