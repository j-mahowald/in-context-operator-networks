'data provider for training and evaluation '
import os
import random
import importlib
...
def print_eqn_caption(equation, caption, num = None, decode = False):
	...
	for inx in range(num):
		...
		if decode:
			print(f'equation {inx}: {this_equation}', flush=True)
			print(f'caption  {inx}: {this_caption}', flush=True)
		else:
			print(f'equation {inx}: {repr(this_equation)}', flush=True)
			print(f'caption  {inx}: {repr(this_caption)}', flush=True)
...
try:
	...
	print(f"tf_rng from FLAGS, seed = {FLAGS.seed + 15}", flush = True)
except AttributeError:
	tf_rng = tf.random.Generator.from_seed(15)
	print(f"tf_rng from default, seed = {15}", flush = True)
...
def select_demo_quest(equation, caption, input_id, embedding_raw, embedding_pool, embedding_mask, cond_k, cond_v, qoi_k, qoi_v, config):
	'randomly select a demo and a quest'
...
def select_caption(equation, caption, input_id, embedding_raw, embedding_pool, embedding_mask, cond_k, cond_v, qoi_k, qoi_v, config):
	...
	if mode == 'random':
		...
		raise ValueError(f'Invalid select_caption: {mode}')
	return equation, caption, input_id, embedding_raw, embedding_pool, embedding_mask, cond_k, cond_v, qoi_k, qoi_v
...
class DataProvider():
	...
	def __init__(self, seed, config, file_names,
		...
		if self.real_time and ('input_id' in self.config['load_list']):
			...
			self.caption_suffix = utils.load_json(f'{caption_home_dir}/{self.caption_dir}/suffix.json')
	...
	def tokenize(self, text_list, max_length):
		'tokenize text_list, return input_ids and attention_mask'
		...
		if encoded_inputs['attention_mask'][:,-1].max() == 1:
			...
			print(f'WARNING: max_length = {max_length} may not be enough, need at least {needed_length}')
		return encoded_inputs['input_ids'], encoded_inputs['attention_mask']

	def load_caption(self, mode):
		'load caption from files'
		file_dict = {
		...
		caption_dict = {}
		print(f"loading caption for dataset {self.name} from {self.caption_dir}")
		for key, value in file_dict.items():
			try:
				caption_dict[key] = read_lists_from_file(f'{self.caption_home_dir}/{self.caption_dir}/{value}.md', mode)
				print(f'INFO: {key} : {self.caption_home_dir}/{self.caption_dir}/{value}.md found')
			except FileNotFoundError:
				print(f'WARNING: {key} : {self.caption_home_dir}/{self.caption_dir}/{value}.md not found')
				caption_dict[key] = []
...
def split_data(caption, data, demo_num_list, caption_id_list = (-1,)):
	'split data into different demo_num'
...
def test(file_names, config):
	...
	for i in range(10):
		utils.timer.tic(f"select_caption {config['select_caption']}")
		data_provider.get_next_data()
		utils.timer.toc(f"select_caption {config['select_caption']}")
