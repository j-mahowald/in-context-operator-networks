'Runner for training and testing the model '
import pickle
import logging
...
def update_dict(params1, params2):
	'update the params1 with params2, return the updated params1'
	assert isinstance(params1, type(params2))
	if isinstance(params1, dict):
		params1.update(params2)
	elif isinstance(params1, FrozenDict):
		p = dict(params1['params'])
		p.update(params2['params'])
		params1 = {'params': FrozenDict(p)}
	else:
		raise ValueError(f'params1 type {params1} not implemented')
	return params1
...
class Runner():
	...
	def init_fn(self, optimizer, trainable_mode, devices):
		...
		if trainable_mode == 'all':
			...
			raise ValueError(f'trainable_mode {trainable_mode} not implemented')
	...
	def save(self, save_dir):
		...
		params_path = save_dir + f'/{self.train_step}_params.pickle'
		opt_state_path = save_dir + f'/{self.train_step}_opt_state.pickle'
		...
		logging.info('saved to %s, step %d', save_dir, self.train_step)

	def restore(self, save_dir, step, restore_opt_state = True):
		'''restore the model from the saved directory'''
		params_path = save_dir + f'/{step}_params.pickle'
		opt_state_path = save_dir + f'/{step}_opt_state.pickle'
		...
		logging.info('restored params from %s, step %d', save_dir, step)
...
class Runner_vanilla(Runner):
	...
	def __init__(self, seed, model, data, model_config, optimizer, trainable_mode, devices = jax.devices(), print_model = True):
		...
		if model == 'icon':
			from models import models_icon as models
...
class Runner_lm(Runner):
	'''Runner for ICON with language model'''
	def __init__(self, seed, model, data, model_config, optimizer, trainable_mode, loss_mode, devices = jax.devices(), print_model = True):
		...
		if model == 'icon_lm':
			from models import models_lm as models
	...
	def _build_loss_fn(self, forward_with_caption_fn, forward_without_caption_fn, predict_with_caption_fn, predict_without_caption_fn, loss_mode):
		...
		if "consist" in loss_mode:
		...
		elif ('cap' in loss_mode) and ('nocap' in loss_mode):
			@jax.jit
			def loss_fn(params, rng_key, data, label):
				loss_nocap = _loss_fn_nocap(params, rng_key, data, label)
			...
			raise ValueError(f'loss_mode {loss_mode} not implemented')
		return loss_fn
