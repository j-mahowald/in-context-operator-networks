...
def main(argv):
  ...
  restore_dir = '/work2/09989/jmahowald/frontera/in-context-operator-networks/icon_lm/save/user/ckpts/icon_weno/{}'.format(FLAGS.ckpt)
  ...
  for gid, bid in zip([0,0,0,0], [0,1,2,3]):
    plt.figure(figsize=figsize)
    for i in range(6):
      idx = i * 10
      alpha = 1.0
      if i <=1:
        label = 'example data' if i == 0 else None
        plt.plot(x, sols[(real_name, 'sim')][gid,bid,idx,:,0], 'k-.', alpha = alpha, label = label)
      if i > 1:
        label = 'ground truth' if i == 2 else None
        plt.plot(x, sols[(real_name, 'sim')][gid,bid,idx,:,0], 'k-', alpha = alpha, label = label)
        label = 'prediction' if i == 2 else None
        plt.plot(x, sols[(real_name, stride, mode)][gid,bid,idx,:,0], 'r--', alpha = alpha, label = label)
    ...
    plt.tight_layout()
    plt.savefig(f'{folder}/profile_one_forward_{folder}_stride{stride}_{mode}_gid{gid}_bid{bid}.pdf')
  ...
  for gid, bid in zip([0,0,0,0], [0,1,2,3]):
    ...
    for i in range(6):
      ...
      if i <=1:
        label = 'example data' if i == 0 else None
        plt.plot(x, sols[(real_name, stride, mode)][gid,bid,idx,:,0], 'k-', label = label)
      if i > 1:
        label = 'prediction' if i == 2 else None
        plt.plot(x, sols[(real_name, stride, mode)][gid,bid,idx,:,0], 'r-', label = label)
      plt.xlabel('$x$')
      plt.ylabel('$u$')
      if bid == 0:
        plt.legend()
    plt.tight_layout()
    plt.savefig(f'{folder}/profile_one_backward_pred_{folder}_stride{stride}_{mode}_gid{gid}_bid{bid}.pdf')
    ...
    for i in range(6):
      ...
      if i == 5:
        label = 'prediction at $t=0$'
        plt.plot(x, sols[(real_name, stride, mode)][gid,bid,idx,:,0], 'r-', label = label)

      label = 'forward simulation' if i == 5 else None
      alpha = 1.0 if i in [0,1,5] else 0.5
      plt.plot(x, sols[(real_name, stride, mode, real_name)][gid,bid,50-idx,:,0], 'b--', alpha = alpha, label = label)
      ...
      if bid == 0:
        plt.legend()
    plt.tight_layout()
    plt.savefig(f'{folder}/profile_one_backward_sim_{folder}_stride{stride}_{mode}_gid{gid}_bid{bid}.pdf')
