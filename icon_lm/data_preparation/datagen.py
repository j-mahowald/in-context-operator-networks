'Main data generation script for generating data for the neural network training'
import os
import sys
import pickle
...
def generate_ode_auto_const(seed, eqns, quests, length, dt, num, caption_mode, name):
	...
	for i, (coeff_a, coeff_b) in enumerate(zip(coeffs_a, coeffs_b)):
		for j in range(quests):
			...
			all_us.append(traj)
			all_params.append(f"{coeff_a:.8f}_{coeff_b:.8f}")
...
def generate_ode_auto_linear2(seed, eqns, quests, length, dt, num, caption_mode, name):
	...
	for i, (coeff_a1, coeff_a2, coeff_a3) in enumerate(zip(coeffs_a1, coeffs_a2, coeffs_a3)):
		for j in range(quests):
			...
			all_cs.append(control)
			all_us.append(traj)
			all_params.append(f"{coeff_a1:.8f}_{coeff_a2:.8f}_{coeff_a3:.8f}")
			all_eqn_captions.append(None)
		print_dot(i)
	for ptype in ['forward', 'inverse']:
...
def generate_pde_poisson(seed, eqns, quests, length, dx, num, caption_mode, name):
	...
	for i, (coeff_ul, coeff_ur) in enumerate(zip(coeffs_ul, coeffs_ur)):
		for j in range(quests):
			...
			all_params.append(f"{coeff_ul:.8f}_{coeff_ur:.8f}")
			all_eqn_captions.append(None)
		print_dot(i)
	for ptype in ['forward', 'inverse']:
...
def generate_pde_porous(seed, eqns, quests, length, dx, num, caption_mode, name):
	...
	for i, (coeff_ul, coeff_ur, coeff_c, coeff_a) in enumerate(zip(coeffs_ul, coeffs_ur, coeff_cs, coeff_as)):
		for j in range(quests):
			...
			us = pdes.solve_linRD_batch(L, N, coeff_ul, coeff_ur, coeff_a * lamda, ks[:,1:-1], coeff_c) # (num, N+1)
			all_xs.append(einshape("i->jik", xs, j = num, k = 1)) # (num, N+1, 1)
...
def generate_pde_cubic(seed, eqns, quests, length, dx, num, caption_mode, name):
	...
	for i, (coeff_ul, coeff_ur, coeff_k, coeff_a) in enumerate(zip(coeffs_ul, coeffs_ur, coeff_ks, coeff_as)):
		for j in range(quests):
			...
			[us,cs] = pdes.solve_nonlinRD_batch(L, N, us_GP, coeff_ul, coeff_ur, coeff_a * lamda, coeff_k)
			all_xs.append(einshape("i->jik", xs, j = num, k = 1)) # (num, N+1, 1)
			all_cs.append(einshape("ij->ijk", cs, k = 1)) # (num, N+1, 1)
			all_us.append(einshape("ij->ijk", us, k = 1)) # (num, N+1, 1)
...
def generate_mfc_gparam_hj(seed, eqns, quests, length, dx, dt, nu_nx_ratio, num, caption_mode, name):
	...
	for i, g in enumerate(gs):
		for j in range(quests):
			...
			all_params.append(f"{g[0]:.8f}_{g[nu//10]:.8f}_{g[2*nu//10]:.8f}_{g[3*nu//10]:.8f}_{g[4*nu//10]:.8f}_{g[5*nu//10]:.8f}_{g[6*nu//10]:.8f}_{g[7*nu//10]:.8f}_{g[8*nu//10]:.8f}_{g[9*nu//10]:.8f}")
			all_eqn_captions.append(None)
		print_dot(i)
...
def generate_pde_lin2d(seed, eqns, quests, length, dx, dt, num, caption_mode, name):
	'''
	...
	for i in range(eqns):
		...
		for _ in range(quests):
			...
			_, gs = pdes.solve_lin2d_batch(L, L, N, N, uxts_GP, coeffs_pass)
	...
	for ptype in ["forward", "inverse"]:
		datawrite.write_pde_lin2d(name=name, eqn_type="pde_linear_3d",
																	all_params=all_params, all_eqn_captions=all_eqn_captions,
																	all_xts=all_xts, all_gs=all_gs, all_uxts=all_uxts,
																	problem_type=ptype)

def generate_pde_heat_homog(seed, eqns, quests, domlen, trajlen, num, name):
	'''
	homogeneous heat equation in spatial 1D and time
	u_t = k*u_xx, x in [0,L], t>0
	u(0,t) = ul, u(L,t) = ur,
	u(x,0) = f(x)
	'''
	key, subkey1, subkey2 = jax.random.split(key=jax.random.key(seed), num=3)
	coeffs_k = jax.random.uniform(key=subkey1, shape=(eqns,), minval=0.001, maxval=0.01)
	coeffs_ul, coeffs_ur = jax.random.uniform(key=subkey2, shape=(2, eqns), minval=-1.0, maxval=1.0)
	time_step = 0.1
	all_xs = []; all_us = []; all_params = []
	for i, (coeff_k, coeff_ul, coeff_ur) in enumerate(zip(coeffs_k, coeffs_ul, coeffs_ur)):
		for j in range(quests):
			key, subkey_q = jax.random.split(key=key, num=2)
			...
			us = pdes.solve_heat_homog_batch(fs, domlen, trajlen, coeff_k, coeff_ul, coeff_ur)
			all_xs.append(einshape("i->jik", xs, j = num, k = 1)) # (num, trajlen+1, 1)
			all_us.append(einshape("ijk->ijkl", us, l=1)) # (num, trajlen, trajlen, 1)
			all_params.append(f"{coeff_k:.8f}_{coeff_ul:.8f}_{coeff_ur:.8f}_q{j}")
	datawrite.write_pde_heat(name, eqn_type='pde_heat_homog', all_params=all_params, all_xs=all_xs,
							  all_us=all_us, problem_type='forward', time_step=int(time_step/(domlen/trajlen)))

def generate_pde_heat_nonhomog(seed, eqns, quests, domlen, trajlen, num, name):
	'''
	non-homogeneous heat equation in spatial 1D and time
	u_t = k*u_xx + au, x in [0,L], t>0
	u(0,t) = ul, u(L,t) = ur,
	u(x,0) = f(x)
	'''
	key, subkey1, subkey2, subkey3 = jax.random.split(key=jax.random.key(seed), num=4)
	coeffs_k = jax.random.uniform(key=subkey1, shape=(eqns,), minval=0.001, maxval=0.01)
	coeffs_a = jax.random.uniform(key=subkey2, shape=(eqns,), minval=-0.01, maxval=-0.001)
	coeffs_ul, coeffs_ur = jax.random.uniform(key=subkey3, shape=(2, eqns), minval=-1.0, maxval=1.0)
	time_step = 0.1
	all_xs = []; all_us = []; all_params = []
	for i, (coeff_k, coeff_a, coeff_ul, coeff_ur) in enumerate(zip(coeffs_k, coeffs_a, coeffs_ul, coeffs_ur)):
		for j in range(quests):
			key, subkey_q = jax.random.split(key=key, num=2)
			xs = jnp.linspace(0.0, domlen, trajlen)# (trajlen,)
			fs = dutils.generate_gaussian_process(subkey_q, xs, num, kernel = dutils.rbf_kernel_jax, k_sigma = 1.0, k_l = 0.5) # (num, N+1)
...
def main(argv):
	...
	if 'pde_lin2d' in FLAGS.eqn_types:
		generate_pde_lin2d(seed=FLAGS.seed, eqns=FLAGS.eqns, quests=FLAGS.quests, length=FLAGS.length,
								caption_mode = FLAGS.caption_mode, dx=FLAGS.dx, dt=FLAGS.dt, num=FLAGS.num, name=name)

	if 'pde_heat_homog' in FLAGS.eqn_types:
		generate_pde_heat_homog(seed=FLAGS.seed, eqns=FLAGS.eqns, quests=FLAGS.quests,
						  		domlen=1.0, trajlen=FLAGS.length, num=FLAGS.num, name=name)

	if 'pde_heat_nonhomog' in FLAGS.eqn_types:
		generate_pde_heat_nonhomog(seed=FLAGS.seed, eqns=FLAGS.eqns, quests=FLAGS.quests,
						  		domlen=1.0, trajlen=FLAGS.length, num=FLAGS.num, name=name)
