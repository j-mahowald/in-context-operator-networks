'This file contains the IconGPT model, which is a transformer-based model for Icon data'
import sys
sys.path.append('..')
...
import utils
from models import models_utils as mutils
...
def build_matrices_from_data_shape(data_shape, config, compact, mode, caption_len, shot_num_min, return_shape_list = False):
	...
	cond_bool_list, qoi_kv_bool_list, qoi_k_bool_list = mutils.build_bool_sequence(demo_num, mode, shot_num_min)
	...
	basic_mask = mutils.build_basic_mask(cond_len_list = cond_len_list,
																qoi_kv_len_list = qoi_kv_len_list,
																qoi_k_len_list = qoi_k_len_list,
																compact = compact)
	if config['index_mode'] == 'learn':
		index_matrix = mutils.build_index_integer(cond_len_list= cond_len_list,
																			qoi_kv_len_list = qoi_kv_len_list,
																			qoi_k_len_list = qoi_k_len_list)
	else:
		raise ValueError(f'not supported index mode: {config["index_mode"]}')

	out_mask = mutils.build_out_mask(cond_len_list = cond_len_list,
													qoi_kv_len_list= qoi_kv_len_list,
													qoi_k_len_list = qoi_k_len_list,
...
class IconGPTModel(nn.Module):
	...
	def basic_forward(self, data, mode, index_matrix, basic_mask, caption_len, shot_num_min):
		...
		cond_bool_list, qoi_kv_bool_list, qoi_k_bool_list = mutils.build_bool_sequence(demo_num, mode = mode, shot_num_min = shot_num_min)
		sequence = mutils.build_data_sequence(data, cond_bool_list, qoi_kv_bool_list, qoi_k_bool_list)

		if self.config['index_mode'] == 'learn':
			sequence = self.pre_projection(sequence)
			sequence = sequence + self.func_pos_embedding(index_matrix)
		else:
			raise ValueError(f'not supported index mode: {self.config["index_mode"]}')
		...
		if caption_len > 0:
			if self.config['caption_feature'] == 'embedding':
				...
				raise ValueError(f'not supported caption feature: {self.config["caption_feature"]}')
			sequence = jnp.concatenate([projected_caption, sequence], axis = 0)
		mask = mutils.build_data_mask(data, cond_bool_list, qoi_kv_bool_list, qoi_k_bool_list)
		if caption_len > 0:
...
def build_network_fn(data, key, config, return_model = False, compact = True, print_model = True):
	...
	basic_mask_with_caption, index_matrix_with_caption, out_mask_with_caption = \
		build_matrices_from_data_shape(data_shape, config, compact,
										mode = 'train', caption_len = config['caption_len'], shot_num_min = 0)
	basic_mask_without_caption, index_matrix_without_caption, out_mask_without_caption = \
		build_matrices_from_data_shape(data_shape, config, compact,
...
def test():
	...
	fig = mutils.plot_model_consts(model.basic_mask_with_caption, model.index_matrix_with_caption, model.out_mask_with_caption)
	fig.savefig('mask_and_index_matrix_icon_gpt_forward_with_caption.pdf')

	fig = mutils.plot_model_consts(model.basic_mask_without_caption, model.index_matrix_without_caption, model.out_mask_without_caption)
	fig.savefig('mask_and_index_matrix_icon_gpt_forward_without_caption.pdf')

	basic_mask, index_matrix, out_mask = \
		build_matrices_from_data_shape(data_shape = tree.tree_map(lambda x: x[0,0].shape, data),
										config = freeze(model_config), compact = True,
										mode = 'test', caption_len = model_config['caption_len'], shot_num_min = 0)
	fig = mutils.plot_model_consts(basic_mask, index_matrix, out_mask)
